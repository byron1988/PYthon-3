def obter_mais_longa_substring(s):
    ...
    caract = s[0] #Pega a primeira letra da string s para iniciar as comparações
    substring = '' #Conterá as substrings
    for x in s[1::]: #Faz iteração em cada letra da string s começando no segundo elemento
        if caract <= x: #Compara para ver se caract é menor (vem antes no alfabeto) do que a próxima letra da string (contida em x)
            if substring == '': #Se caract vier antes de x,  verifica se a variável substring está vazia
                substring += caract + x #Se estiver, adiciona caract e x a variál substring
            else:
                substring += x #Se já contiver algo, só x será adicionado
        ...
        caract = x #Atualiza a variavel caract com a próxima letra da string s (contido em x)
    print(maior_substring)
    return maior_substring #retorna a maior substring
obter_mais_longa_substring("elemento")
